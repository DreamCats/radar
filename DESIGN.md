...
## radar 适配说明（在 Linear 基底上的本地化）

> 本文件以 awesome-design-md 的 **Linear** DESIGN.md 为基底，针对 radar（个人投研工作台）做了金融场景扩展。
> 选型理由与评估见 `docs/06-design-eval.md`。

### 为什么基于 Linear
- 极黑底（`#010102`）+ 单一薰衣草蓝点缀，**信息密度极高、克制耐看**，最接近专业投研终端的气质。
- "dense, technical, quietly luxurious" 的定位天然契合「每天盯消息流 + 数据看板」。
...
3. **承袭 Linear 原则**：薰衣草蓝仍是唯一品牌强调色（选中、focus、主 CTA）；
   涨跌色与信号色属于「数据语义层」，不与品牌色抢戏。
