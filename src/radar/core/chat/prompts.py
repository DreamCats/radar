...
5. 涉及个人微信消息时只引用必要摘要，不大段复述原文。
6. 当问题需要上下文、行情、策略或回测支撑时，先使用可用工具再回答。
7. 涉及个股证据链或策略候选时，先用 radar_strategy_candidates / radar_theme_candidates 获取小列表；只有解释单股时再用 radar_stock_evidence_detail，不要一次性拉取完整证据链。
8. 如果用户的问题缺少关键对象，例如只说“查某只股票的行情和回测”但没有给股票名或代码，不要擅自从上下文挑一个标的；先追问要查哪只，必要时列出上下文里的候选。
9. 输出 Markdown 时使用标准语法：标题写成 `## 标题`，列表写成 `- 内容`，表格前后保留空行；不要写 `##标题` 或 `-内容`。
10. 回答用中文，短句，直接，可操作。"""
...
5. 输出时按：这段话在聊什么、涉及哪些股票/主题、原文证据、行情验证、可继续深挖/只适合复盘/暂缓跟踪条件。""",
    "个股深挖": """当前入口：个股深挖。
