...
from radar.core.usecases.catalyst_strategy import CatalystStrategyRunResult, run_catalyst_strategy_report


@click.group("catalyst-strategy")
def catalyst_strategy() -> None:
    """基于催化词线索生成策略报告。"""


@catalyst_strategy.command("run")
...
@click.option("--llm-concurrency", type=click.IntRange(1, 8), default=3, show_default=True, help="LLM 按标的并发数。")
@click.option("--provider", "provider_name", help="指定 LLM provider。")
...
def _echo_result(result: CatalystStrategyRunResult, *, output_format: str) -> None:
